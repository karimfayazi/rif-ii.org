 ...
 */
function getLayerInfo(filename) {
 ...
 */
async function convertShapefile(baseName, sourceDir, outputDir) {
  try {
...
async function findShapefiles(dir) {
  ...
  async function scan(currentDir) {
    try {
      ...
      console.warn(`  Warning: Could not read directory ${currentDir}: ${error.message || 'Unknown error'}`);
    }
  }
...
async function main() {
  ...
  try {
    ...
    console.error(`ERROR: Could not create output directory: ${error.message || 'Unknown error'}`);
    process.exit(1);
  }
  ...
  // Write manifest
  const manifestData = { layers: manifest };
