...
import { MapPin, Droplet, Trash2, Map as MapIcon, Layers, Satellite, Building2, ChevronDown } from 'lucide-react';
				...
				try {
					...
					const loadLayer = async (mapItem: MapType) => {
						try {
							...
							console.log(`Loading layer: ${mapItem.name} from ${filePath}`);
							const response = await fetch(filePath);
							if (!response.ok) {
								console.error(`Failed to load ${mapItem.name} from ${filePath}:`, response.status, response.statusText);
								return;
							}
...
export default function KMLGISMapsPage() {
	...
	const allLayers = [
		...additionalLayers.filter(map => map.id === 'kpk-districts'),
		...additionalLayers.filter(map => map.id === 'dik-tehsil'),
		...panialaMaps
	];
	...
	return (
		<div className="space-y-6">
			{/* Header */}
			<div>
				<h1 className="text-3xl font-bold text-gray-900">GIS Online Maps</h1>
			</div>
