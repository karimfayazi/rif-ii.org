...
const PAHARPUR_KMZ_DIRECTORY = path.join(
	process.cwd(),
	"public",
	"maps",
	"kmz",
	"Paharpur_kmz_gis_maps",
);
...
export async function GET() {
	try {
		const directoryEntries = await fs.readdir(PAHARPUR_KMZ_DIRECTORY, { withFileTypes: true });
		...
		const results = await Promise.all(
			kmzFiles.map(async (entry) => {
				...
				try {
					const fullPath = path.join(PAHARPUR_KMZ_DIRECTORY, entry.name);
