...
const DOMEL_KMZ_DIRECTORY = path.join(
	process.cwd(),
	"public",
	"maps",
	"kmz",
...
export async function GET() {
	try {
		const directoryEntries = await fs.readdir(DOMEL_KMZ_DIRECTORY, { withFileTypes: true });
		...
		const results = await Promise.all(
			kmzFiles.map(async (entry) => {
				...
				try {
					const fullPath = path.join(DOMEL_KMZ_DIRECTORY, entry.name);
